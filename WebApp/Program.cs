...
{
    // Initialize dependency injecton container, get logger, wait for DB conn
    ...
    // Ask for what is said in settings
...
static void WaitDbConnection(AppDbContext ctx, ILogger<IApplicationBuilder> logger)
{
    while (true)
    {
        ...
        {
            logger.LogWarning("Postgres server not reachable yet: {}. Sleep 1 sec", e.Message);
            System.Threading.Thread.Sleep(1000);
        }
    }
}